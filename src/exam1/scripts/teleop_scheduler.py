...
class TurtleTeleop(Node):
    def __init__(self):
        ...
        # Declare max_pizzas parameter
        max_pizzas_descriptor = ParameterDescriptor(description='Maximum number of pizzas allowed')
        ...
        self.manual_cmd_pub = self.create_publisher(Twist, 'controller/manual_cmd', 10)
        ...
        # Movement configuration - keep exactly as original
        self.move_dirs = {
            'w': [3.0, 0.0],
            's': [-3.0, 0.0],
            'a': [0.0, 3.0],
    ...
    def send_manual_command(self, linear_vel, angular_vel):
        """Send manual control command to local controller"""
        msg = Twist()
    ...
    def timer_callback(self):
        # Keep original logic exactly - just use local controller communication
    ...
    def spawn_pizza(self, x, y):
        if len(self.static_queue) >= self.max_pizzas or self.save_count >= self.max_saves:
    ...
    def clear_pizzas(self):
        ...
        # Send target to local controller
        target = self.pizza_queue[0]
        if not self.target_reached:
            self.send_target(target[0], target[1])
        
        # Check if controller says we reached the target
